...
Env::Env(const char** argv)
{
    ...
    //get initial qpos and qdata
    mj_resetData(Env::m,Env::d);
    for(size_t i=0; i<Env::m->nq; i++)
        {Env::initial_qpos.push_back(d->qpos[i]);
        }
    
    for(size_t i=0; i<Env::m->nv; i++)
        Env::initial_qvel.push_back(d->qvel[i]);
    
    Env::state_size = Env::m->nq + Env::m->nv + Env::m->nu;
    Env::action_size = Env::m->nu;


}
void Env::reset(vector<double> &state)
{
    mj_resetData(Env::m,Env::d);
    for(short i=0; i<Env::m->nq; i++)
        d->qpos[i] = Env::initial_qpos[i] + 0.1*(((double) rand() / (RAND_MAX))-0.5);
    for(short i=0; i<Env::m->nv; i++)
        d->qvel[i] = Env::initial_qvel[i] + 0.1*(((double) rand() / (RAND_MAX))-0.5);
...
void Env::get_state(vector<double> &state)
{
    ...
    for(short i=0; i<Env::m->nv; i++)
        state[i+Env::m->nq] =  d->qvel[i];
    
    for(short i=0; i<Env::m->nu; i++)
        state[i+(Env::m->nq+Env::m->nv)] =  d->ctrl[i];

    //std::this_thread::sleep_for(std::chrono::microseconds(1));

}

void Env::get_reward(vector<double> &state, vector<double> &new_state, vector<double> &action, double &reward)
{
    //half cheetah reweard
    double action_penalty = 0.0;
    for(size_t i=0; i<action.size(); i++)
    {
        action_penalty += action[i]*action[i];
    }
    reward = new_state[0] - state[0] - 0.002*action_penalty;
}
